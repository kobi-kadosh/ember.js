...
QUnit.test('it should return false if no target or action are specified', function () {
  ...
  var obj = _emberRuntimeSystemObject2['default'].extend(_emberRuntimeMixinsTarget_action_support2['default']).create();
...
QUnit.test('it should support actions specified as strings', function () {
  ...
  var obj = _emberRuntimeSystemObject2['default'].extend(_emberRuntimeMixinsTarget_action_support2['default']).create({
    target: _emberRuntimeSystemObject2['default'].create({
...
QUnit.test('it should find targets specified using a property path', function () {
  ...
  var Test = {};
  _emberMetalCore2['default'].lookup = { Test: Test };

  Test.targetObj = _emberRuntimeSystemObject2['default'].create({
    anEvent: function anEvent() {
      ok(true, 'anEvent method was called on global object');
    }
  });

  var myObj = _emberRuntimeSystemObject2['default'].extend(_emberRuntimeMixinsTarget_action_support2['default']).create({
...
QUnit.test('it should use the target specified in the argument', function () {
  expect(2);
  var targetObj = _emberRuntimeSystemObject2['default'].create({
    anEvent: function anEvent() {
      ok(true, 'anEvent method was called');
    }
  });
...
QUnit.test('it should allow multiple arguments from actionContext', function () {
  ...
  var obj = _emberRuntimeSystemObject2['default'].extend(_emberRuntimeMixinsTarget_action_support2['default']).create({
    target: _emberRuntimeSystemObject2['default'].create({
      anEvent: function anEvent(first, second) {
