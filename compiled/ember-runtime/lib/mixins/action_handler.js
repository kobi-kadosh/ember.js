...
var ActionHandler = _emberMetalMixin.Mixin.create({
  ...
  */
  willMergeMixin: function willMergeMixin(props) {
    ...
    if (!props._actions) {
      _emberMetalCore2['default'].assert('\'actions\' should not be a function', typeof props.actions !== 'function');

      if (!!props.actions && typeof props.actions === 'object') {
        hashName = 'actions';
      } else if (!!props.events && typeof props.events === 'object') {
        _emberMetalCore2['default'].deprecate('Action handlers contained in an `events` object are deprecated in favor' + ' of putting them in an `actions` object', false);
        hashName = 'events';
      }

      if (hashName) {
        props._actions = (0, _emberMetalMerge2['default'])(props._actions || {}, props[hashName]);
      }
