...
var Test = {
  ...
  */
  registerHelper: function registerHelper(name, helperMethod) {
    helpers[name] = {
      method: helperMethod,
      meta: { wait: false }
    };
  ...
  /**
    ...
    @since 1.2.0
  */
  registerAsyncHelper: function registerAsyncHelper(name, helperMethod) {
    helpers[name] = {
      method: helperMethod,
      meta: { wait: true }
    };
  ...
  */
  onInjectHelpers: function onInjectHelpers(callback) {
  ...
  */
  promise: function promise(resolver, label) {
  ...
  */
  ...
  /**
     ...
     @since 1.2.0
  */
  registerWaiter: function registerWaiter(context, callback) {
    if (arguments.length === 1) {
      callback = context;
      context = null;
    }
    if (!this.waiters) {
      this.waiters = _emberMetalCore2['default'].A();
    }
  ...
  /**
     ...
     @since 1.2.0
  */
  unregisterWaiter: function unregisterWaiter(context, callback) {
    if (!this.waiters) {
      return;
    }
    if (arguments.length === 1) {
    ...
    this.waiters = _emberMetalCore2['default'].A(this.waiters.filter(function (elt) {
...
function run(fn) {
  if (!_emberMetalRun_loop2['default'].currentRunLoop) {
    return (0, _emberMetalRun_loop2['default'])(fn);
  } else {
    return fn();
  }
}

_emberApplicationSystemApplication2['default'].reopen({
  /**
...
Test.Promise = function () {
  _emberRuntimeExtRsvp2['default'].Promise.apply(this, arguments);
  Test.lastPromise = this;
};

Test.Promise.prototype = Object.create(_emberRuntimeExtRsvp2['default'].Promise.prototype);
Test.Promise.prototype.constructor = Test.Promise;
Test.Promise.resolve = Test.resolve;

// Patch `then` to isolate async methods
// specifically `Ember.Test.lastPromise`
var originalThen = _emberRuntimeExtRsvp2['default'].Promise.prototype.then;
