...
function createRenderBuffer(tagName, contextualElement) {
  ...
  var buffer = new _emberViewsCompatRender_buffer2['default'](domHelper);
...
QUnit.test('properly handles old IE\'s zero-scope bug', function () {
  ...
  ok((0, _emberViewsSystemJquery2['default'])(element).html().match(/script/i), 'should have script tag');
  ok(!(0, _emberViewsSystemJquery2['default'])(element).html().match(/&shy;/), 'should not have &shy;');
});
